    ...
    if(r.emoji.name === "✨") {
        message.channel.send(`\`${getNow().time}\` ✨ Création de la catégorie des salons personnalisé en cours..`).then(msg => {
            m.guild.channels.create('Tempo', {
                ...
                c.guild.channels.create('🌹・Vocal 1', {
                    type: 'voice',
                    parent: c.id,
                ...
                update(message, db)
                m.edit({ embed: { author: { name: `🕙 Modification des salons temporaires de ${message.guild.name}`}, color: db.color, description:  "`✨` Crée une configuration automatique\n`📖` Modifier la catégorie\n`🏷️` Modifier le salon de création\n`🎗️` Modifier l'emoji en prefixe des salons temporaires\n`✅`Activer le module\n`❌` Désactiver\n\n > [Configuration actuel:](https://discord.gg/4TDSA2UQ9t)" , fields: [ {name: "`📖` Catégorie", value: db.tempo.category, inline: true }, { name: "`🏷️` Salon", value: db.tempo.channel, inline: true},{ name: "`🎗️` Emoji", value: db.tempo.emoji, inline: true}   ] } });         
